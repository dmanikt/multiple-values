...
void print_vals(val_vals_t *vals) {
  print_result_interior(vals->fst);

  switch (val_typeof(vals->snd)) {
  case T_EMPTY:
    break;
  case T_VALS:
    printf("\n");
    print_vals(val_unwrap_vals(vals->snd));
    break;
  default:
    printf("\n");
    print_result_interior(vals->snd);
    break;
  }
}
