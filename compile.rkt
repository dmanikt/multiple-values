...
(define (compile-e e c t?)
  (match e
    ...
    [(Mv-values es)     (compile-values es c)]
    [(Mv-let e ids body) (compile-mv-let e ids body c t?)]
...
(define (compile-values es c)
  (seq (Mov vc (length es))
    (match (length es)
      [0 (Mov rax val-void)]
      [1 (compile-e (car es) c #f)]
      [_ (compile-vals es c)])))

;; [Listof Expr] CEnv -> Asm
(define (compile-vals es c)
  (match es
    ['()
...
; (Mv-let expr ids body)
;; Expr [Listof Ids] Expr CEnv Bool -> Asm
(define (compile-mv-let expr ids body c t?)
  (let ((values (gensym))
        (loop   (gensym))
        (end    (gensym)))
    (seq (compile-e expr c #f)
         (Mov 'r13 rax)
         (And rax ptr-mask)
         (Cmp rax type-vals)
         (Je  values)
         ...
         (Label end)
         (compile-e body (append (reverse ids) c) t?)
         (Add rsp (* 8 (length ids)))
    )
  )
)
